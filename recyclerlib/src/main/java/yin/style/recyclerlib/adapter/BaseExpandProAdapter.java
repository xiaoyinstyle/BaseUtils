 ...
 * @author chenyin
 * @date 2017/3/28
 * 购物车试使用时，多次刷新 出现了卡死bug，暂未解决
 * 如果没有频繁的刷新操作 可以临时使用，BaseExpandAdapter这个基本已经满足了常用需求
 */
public abstract class BaseExpandProAdapter<T> extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    public BaseExpandProAdapter(Context context, List list) {
        this.list = list;
        mContext = context;
    }
        ...
        if (holder instanceof ItemViewHolder) {
            ...
            if (onItemClickLongListener != null)
                holder.itemView.setOnLongClickListener(new View.OnLongClickListener() {
                    ...
                    public boolean onLongClick(View v) {
                        return onItemClickLongListener.onItemLongClick(holder.itemView, position, -1);
                    }
                });
            ...
            ((ItemViewHolder) holder).mGroup.setAdapter(new ItemExpandAdapter(getChild(position), position) {
                ...
                protected int getChildLayout() {
                    return BaseExpandProAdapter.this.getChildLayout(position);
                }

                @Override
                protected void setViewHolder(BaseViewHolder holder, int groupPosition, int childPosition) {
                    BaseExpandProAdapter.this.setChildViewHolder(holder, groupPosition, childPosition);
                }
                ...
                public boolean onItemLongClick(View view, int groupPosition, int childPosition) {
                    if (onItemClickLongListener != null)
                        return onItemClickLongListener.onItemLongClick(holder.itemView, groupPosition, childPosition);
                    return false;
                }
            }));
        }
